...
<body>
  ...
  $sqlProyecto = $conexion->query("SELECT * FROM projects WHERE nm_juridicas_numid = $lastNumber");
  $proyecto = $sqlProyecto->fetch_object();

  // Consulta para obtener los contactos del proyecto
  $sqlContacto = $conexion->query("SELECT * FROM nm_contactos WHERE id_contacto = $lastNumber");
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query = "INSERT INTO projects 
        (DateTime, nm_juridicas_numid, Name, am_monedas_id, CostCenter, IndustrialSectors_Id, np_ciudades_id_ciudad, cp_incoterm_Id_incoterm, np_paises_id_pais, DeliveryTime_Id, vp_terminospago_id_termino, OfferValidity_Id, ip_dtbasicos_sec_basico)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    if ($stmt = $conexion->prepare($query)) {
      ...
      if ($stmt->execute()) {
        $projects_id = $conexion->insert_id;

        // Insertar datos en ProjectsContacts
        $nm_contactos_id_contacto = $_POST['id_contacto'];
        $queryContacts = "INSERT INTO ProjectsContacts 
                (projects_id, nm_contactos_id_contacto, ip_dtbasicos_sec_basico)
                VALUES (?, ?, ?)";
        ...
        if ($stmtContacts = $conexion->prepare($queryContacts)) {
          ...
          if (!$stmtContacts->execute()) {
            echo "Error al agregar el contacto al proyecto: " . $stmtContacts->error;
          }
          $stmtContacts->close();
        } else {
        ...
        // insertar Cliente 
        $queryContacts = "INSERT INTO nm_juridicas 
        (numid, razon_social, industrialsectors_id)
        VALUES (?, ?, ?)";
        if ($stmtClient = $conexion->prepare($queryContacts)) {
          ...
          if (!$stmtClient->execute()) {
            echo "Error al agregar el cliente al proyecto: " . $stmtClient->error;
          }
          $stmtClient->close();
        } else {
          echo "Error al preparar la consulta de clientes: " . $conexion->error;
        }

        // Insertar exclusiones seleccionadas en Projects_Exclusions
        if (isset($_POST['exclusions']) && is_array($_POST['exclusions'])) {
          $queryExclusions = "INSERT INTO Projects_Exclusions 
                    (projects_id, exclusion_id, ip_dtbasicos_sec_basico)
                    VALUES (?, ?, ?)";
          ...
          foreach ($_POST['exclusions'] as $exclusion_id) {
            if ($stmtExclusions = $conexion->prepare($queryExclusions)) {
              ...
              if (!$stmtExclusions->execute()) {
                echo "Error al agregar exclusión: " . $stmtExclusions->error;
              }
        ...
        // Redirigir a la misma página con el ID del nuevo proyecto
        echo '<script type="text/javascript">window.location.href = "crear.php?id=' . ($lastNumber + 1) . '";</script>';
        exit;
      } else {
  ...
  <div class="container-fluid mt-3 align-items-center">
    ...
    <form method="POST">
      ...
      <div class="row mb-2">
        ...
        <div class="col-auto">
          <label class="highlight" id="numero" name="numero"><?= htmlspecialchars($lastNumber) ?></label>
        </div>
        <!-- Nombre del Proyecto -->
        <div class="col-auto">
      ...
      <button type="button" class="btn btn-primary custom-border" onclick="window.location.href='servicio.php?id=<?= $lastNumber ?>'">
